...
class StorageJson(istorage.IStorage):
    ...
    Prints the total number of movies.
        ...
        for movie in range(len(movies)):
            count += 1
            movies_string += f"{movies[movie]['title']}: {movies[movie]['rating']}\n"
    ...
    If the movie doesn't exist, generates the movie information and adds it to the database.
        ...
        load_movie.append(generated_movie)
        self.save_file(load_movie)
        return f"{GREEN}Movie {title} successfully added{RESET}"
    ...
    Saves the updated movie information back to the file.
        ...
        load_movie = self.load_from_file()
        title = input("Enter movie name: ")
        for movie_info in load_movie:
            if movie_info["title"] == title:
                load_movie.remove(movie_info)
                self.save_file(load_movie)
    ...
    def movie_stats(self):
        ...
        for movie_info in movies:
            rating_float_value = float(movie_info["rating"].split("/")[0])
        ...
        # BEST MOVIE
        max_ratings = max([mov_1["rating"] for mov_1 in movies])
        for i in movies:
            if i["rating"] == max_ratings:
                print(f"{BLUE}Best movie: {i['title']}, {max_ratings}{RESET}")

        # WORST MOVIE
        worst_rating = min([mov_2["rating"] for mov_2 in movies])
        for j in movies:
            if j["rating"] == worst_rating:
                print(f"{RED}Worst movie: {j['title']}, {worst_rating}{RESET}")

    def random_movie(self):
        """Use the random module to randomly select a movie from the dict"""
        movies = self.load_from_file()
        movie_random = random.choice(movies)
        return f"{GREEN}Your movie for tonight is: {movie_random['title']}, {movie_random['rating']}{RESET}"
    ...
    def search_movie(self):
        ...
        if found_movies:
            sorted_found_movies = sorted(found_movies, key=lambda x: x["rating"], reverse=True)
            print(f"No exact matches found for '{search}'. Did you mean:")
            for movie in sorted_found_movies:
                print(f"{GREEN}{movie['title']}{RESET}")
        else:
    ...
    def sort_by_rating(self):
        ...
        for movie in sorted_movies:
            movies_str += f"{GREEN}{movie['title']}: {movie['rating']}{RESET}\n"
        return movies_str
    ...
    def web_generator(self):
        ...
        for movie in load_movie:
            movie_str += istorage.web_serialize(movie)
        return movie_str
